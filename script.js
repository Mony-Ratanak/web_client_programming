...
/* ── Interface mock: part selection ── */
window.selectPart=function(part){
  document.getElementById('elm-panel').setAttribute('aria-pressed',part==='panel'?'true':'false');
  document.getElementById('elm-canvas').setAttribute('aria-pressed',part==='canvas'?'true':'false');
  document.getElementById('el-info-default').hidden=true;
  document.getElementById('el-info-panel-txt').hidden=(part!=='panel');
  document.getElementById('el-info-canvas-txt').hidden=(part!=='canvas');
};

/* ── Three Tabs switcher ── */
window.showTab3=function(tab,btn){
  document.querySelectorAll('.ttd-tab').forEach(t=>t.classList.remove('active-tab'));
  btn.classList.add('active-tab');
  ['content','style','adv'].forEach(t=>{
    const el=document.getElementById('ttp-'+t);if(el)el.hidden=(t!==tab);
  });
};
...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  // interface mock keyboard
  ['elm-panel','elm-canvas'].forEach(id=>{
    const el=document.getElementById(id);
    if(el)el.addEventListener('keydown',e=>{
      if(e.key==='Enter'||e.key===' '){e.preventDefault();selectPart(id==='elm-panel'?'panel':'canvas');}
    });
  });
  console.info('[Lab] Week 12 · Session 1 — Total steps:',TOTAL);
});
})();
