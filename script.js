/**
 * Week12_Session3_Lab.js
 * User Roles & Site Backups — ITC Web Client Programming
 */
(function(){
'use strict';
...
const PHASES={
  ...
  3:{total:4, bar:'p3-fill',lbl:'p3-label',dot:'dot-3',nc:'nc-3',bn:'banner-3'},
...
/* ── Role Explorer ── */
const ROLE_IDS=['admin','editor','author','contributor','subscriber'];
window.showRole=function(role, btn){
  // Update button states
  document.querySelectorAll('.re-role').forEach(b=>{
    b.classList.remove('re-selected');
  ...
  // Show correct panel
  ROLE_IDS.forEach(r=>{
    const panel=document.getElementById('re-panel-'+r);
...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  // Set first role as selected on load
  const firstBtn=document.querySelector('.re-role');
  if(firstBtn)firstBtn.classList.add('re-selected');
  ...
  // Role buttons keyboard
  document.querySelectorAll('.re-role').forEach(btn=>{
    btn.addEventListener('keydown',e=>{
      if(e.key==='Enter'||e.key===' '){e.preventDefault();btn.click();}
    });
  });

  console.info('[Lab] Week 12 · Session 3 — Total steps:',TOTAL);
});
})();
